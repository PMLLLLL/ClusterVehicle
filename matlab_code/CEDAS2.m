            ...
            for i = 1:numClusters
                if max(obj.clusters(i).Data(:, 1)) < sample(1) % % 取簇的第一列数据筛选符合条件的簇
                    ...
                    if(length(obj.clusters(i).Data(:, 1)) >= obj.startPreNum)
                        ...
                        distances(i) = EuclideanDistance(obj.weights(1:4),sample(1:4),[pre obj.clusters(i).Centre(4)]);

                        % distances(i) = sqrt(sum((obj.weights(1:4) .* (sample(1:4) - [pre obj.clusters(i).Centre(4)])).^2));
                    else
                        distances(i) = EuclideanDistance(obj.weights(1:4),sample(1:4),obj.clusters(i).Centre(1:4));
