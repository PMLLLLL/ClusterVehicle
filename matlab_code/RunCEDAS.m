...
for WeightNum = 1:4
    ...
    % 聚类处理
    for i = 1:count
        CE = CEDAS(rad, decay,[(1-0.98)/3, 0.98, (1-0.98)/3, (1-0.98)/3], initDirection); % 用于存储聚类结果
        ...
        fprintf('第%d次聚类结果的数量: %d 权重Weights = [ %s] RI = %s \n',...
                i,length(CE.clusters),sprintf('%.2f ', [(1-0.98)/3, 0.98, (1-0.98)/3, (1-0.98)/3]),sprintf('%.4f ', RI(i)));
    end
